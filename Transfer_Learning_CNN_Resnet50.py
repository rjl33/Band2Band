...
}, 'transfer_learning_model_Resnet50.pth')

print("✓ Saved: step3_resnet50_model.pth")
...
model = model.to(device)
checkpoint = torch.load('transfer_learning_model_Resnet50.pth', weights_only=False)
...
# Save results
results = {
    'experiment': 'Step 3: ResNet50 (Isolated)',
    'architecture': 'ResNet50',
    'resolution': '64x64',
    'modifications': 'Architecture: ResNet50 (ONLY change - no class weights, baseline resolution)',
...
ax1.set_title("Step 3: Loss vs. Epoch (ResNet50)", fontsize=14, weight='bold')
ax1.legend()
ax1.grid(alpha=0.3)
...
ax2.set_title("Step 3: Accuracy vs. Epoch (ResNet50)", fontsize=14, weight='bold')
ax2.legend()
ax2.grid(alpha=0.3)

plt.tight_layout()
plt.savefig('step3_training_curves.png', dpi=150, bbox_inches='tight')
print("✓ Saved: step3_training_curves.png")
plt.show()
