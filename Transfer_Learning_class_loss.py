...
#Class weights: 
class_weights = torch.tensor([1.0, 1.0, 1.2, 1.8, 2.5], device=device)
print(f"Class-wise weights: {class_weights.cpu().numpy()}")
print("=" * 50)

#Create a class for custom weighted BCE loss
class WeightedBCEWithLogitsLoss(nn.Module):
    def __init__(self, pos_weight, class_weight):
        super().__init__()
...
# Loss & optimizer
criterion = WeightedBCEWithLogitsLoss(pos_weight=pos_weight, class_weight=class_weights)
...
#     'train_losses': train_losses,      # For plotting later
#     'val_losses': val_losses,          # For plotting later
#     'train_accs': train_accs,          # For plotting later
#     'val_accs': val_accs,              # For plotting later
#     'class_weights': class_weights.cpu().numpy(),
# }, 'Transfer_Learning_class_loss.pth')
...
model = model.to(device)
checkpoint = torch.load('Transfer_Learning_class_loss.pth', weights_only=False)
...
results = {
    'experiment': 'Step 1: Class-Weighted Loss',
    'architecture': 'ResNet18',
    'resolution': '64x64',
    'modifications': 'Class weights: [1.0, 1.0, 1.2, 1.8, 2.5]',
